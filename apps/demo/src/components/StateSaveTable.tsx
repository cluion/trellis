...
interface StateSaveTableProps {
  data: User[];
}

export function StateSaveTable({ data }: StateSaveTableProps) {
  ...
  const { api } = useTrellis<User>({
    ...
    plugins: [
      ...
      createStateSavePlugin({
        key: 'demo-state-save',
        saveFields: ['sorting', 'filtering', 'pagination', 'columnVisibility', 'expandedRows'],
      }),
    ],
  });
  ...
  return (
    <div>
      ...
      <div className="status-bar">
        ...
        <span>
          <button
            ...
            onClick={() => api.emit('state:save', undefined)}
            style={{ marginRight: 4 }}
          >
            立即保存
          </button>
          <button
            className="btn btn-sm"
            onClick={() => api.emit('state:clear', undefined)}
          >
            清除保存
          </button>
        </span>
      ...
      <p className="description" style={{ marginTop: 12, fontSize: 13, color: '#6b7280' }}>
        提示：排序、篩選、分頁狀態會自動保存到 localStorage。重新整理頁面後狀態會自動恢復。
      </p>
    </div>
  );
}
